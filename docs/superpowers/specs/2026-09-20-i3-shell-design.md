...
- `windows.ts` — `Meta.Window` ⇄ `WindowId`; connects `window-created`, per-window `unmanaged`, `size-changed`, `position-changed`, `focus`, `workspace-changed`, `notify::minimized`, `notify::maximized-horizontally`, `notify::maximized-vertically`, `notify::fullscreen`, `notify::on-all-workspaces`, `notify::skip-taskbar`, and the window actor's `first-frame`; emits plain events to the engine. The last two are the per-commit membership facts of §8.2/§8.3 and are disposed with every other window handler.
...
Every engine gate that asks "is this window ours to place" consults this one predicate, not `minimized` alone: the `tree.normalize()` live set, the forced-generation check when a window rejoins, the leave/rejoin branch, the `unmaximize()` request for a window found maximized, and `_observe()`'s corrective re-apply. The last two were missed when the predicate was introduced, and the `unmaximize()` one made the extension undo the user's own maximize on a pinned window, once per maximize, forever, without ever being able to tile it — active interference with a window this rule declares not ours (fixed 2026-09-23, whole-branch review).

`null` from `classifyWindow` now means exactly one thing: **this window type is not ours.** That closes the permanent drop *in practice*, not by construction: `type` is itself read once and X11 clients may rewrite `_NET_WM_WINDOW_TYPE` after mapping, so an `'ignored'` window promoted to NORMAL after its first frame would still be dropped permanently and silently, its watch already disposed. That residual hole was assessed and accepted for v1 before Phase 3B (Phase 1 carry-forward) and is Phase 4's to close with per-commit reclassification; it is recorded here so the guarantee is not mistaken for an absolute one. The tracker allocates an id and keeps its watch for every window it admits, and re-entry needs no new lifecycle state — a window that is excluded is still tracked, and rejoins the tree beside the focused window per §7.3 the moment every reason has cleared (§7.10 records what that pins). Both facts are watched natively, through `notify::on-all-workspaces` and `notify::skip-taskbar` (§8.3); without a signal a flipped fact would sit unnoticed until some unrelated commit happened to re-read it, so a window would leave or rejoin the tiling at an arbitrary later moment rather than when the user acted.
...
2. **Geometry reconciliation.** Each `geometry.apply` records the window's expected rect. A changed expected rect starts a new reconciliation generation. On `size-changed` / `position-changed`, a tiled window whose frame differs from that rect receives at most one corrective re-apply for that generation, through `commit()`. Matching notifications do nothing. If the frame still differs after that corrective application has taken effect, mark the window stubborn for that generation and stop re-applying until a later commit changes its expected rect. Duplicate notifications do not reset the retry allowance. This bounds retries for a client whose minimum size exceeds its tile; fixed-size normal windows float instead (§8.2).
   ...
   Four kinds of lifecycle change force a fresh application and reconciliation generation even when the expected rect is unchanged: fullscreen exit (`notify::fullscreen` → false), **a window rejoining the tree — any exclusion reason of §8.2 clearing, not unminimize alone** (`excludedFromTree(old) && !excludedFromTree(info)`, so `notify::minimized` → false, `notify::on-all-workspaces` → false and `notify::skip-taskbar` → false all count, and a window that clears one reason while another still holds forces nothing because it is still out of the tree), `monitors-changed`, and completion of the engine's `unmaximize()` after catching a tiled window maximizing (`notify::maximized-horizontally` / `-vertically`). These invalidations bypass the normal rect diff and the old generation's stubborn marker. Fullscreen and minimized windows are not held to tiled geometry while in those states. `workspace-changed` is not a forced-reapply exception; its normal tree mutation and rect diff suffice. All re-applications go through the single commit pipeline, never directly from signal handlers.
