...
A window is **tiled** iff all of: `window_type == NORMAL`; not `skip_taskbar`; `get_transient_for() == null`; not `is_attached_dialog()`; not on all workspaces; **intrinsically resizable**; not matched by a `floating enable` rule (Phase 4).
...
- Scenarios: A8–A14 from §3, adoption of a window that is mapped maximized and of one that is mapped fullscreen (§8.2), lock/unlock (`org.i3shell.Debug.SimulateSessionMode`), relayout, and a disable/enable cycle.
