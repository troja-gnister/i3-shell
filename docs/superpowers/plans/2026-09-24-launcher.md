...
> **Correction (2026-09-24, after the whole-branch review):** this plan states in
> several places that `src/shell/**` cannot be unit-tested because
> `tsconfig.test.json` excludes it. That is wrong. The exclusion is a *typecheck*
> boundary. The suite reaches adapters by mocking the GNOME modules — see
> `test/unit/shell/bars.test.ts` and the fakes in `test/unit/shell/fakes/` — and
> `test/unit/shell/launcher.test.ts` and `appCatalogue.test.ts` now do exactly
> that. Where a task below defers something "because the adapter cannot be
> tested", the reasoning is void even where the conclusion happened to be sound.
...
`src/shell/**` is excluded from `tsconfig.test.json` and cannot be unit-tested on Node, so everything decidable without GNOME lives in `src/launcher/paths.ts` and is tested there. *(False premise — see the correction under Global Constraints. `test/unit/shell/appCatalogue.test.ts` now covers this adapter directly, including the `$PATH` cache's `current` map.)* That is the same split `src/shell/rowHeight.ts` and `src/runtime/classify.ts` already use.
...
This task has no Node unit test — `src/shell/**` is excluded from `tsconfig.test.json` by design. Its gate is Task 9's native scenario plus the live walk. *(False premise, and the most costly of the three — see the correction under Global Constraints. `test/unit/shell/launcher.test.ts` exists and pins this task's whole output, starting with the order of the grab teardown.)* Write it so that the parts that could have been decided here were decided in Tasks 1–3 instead.
